<template>
  <div class="glossy-card rounded-2xl p-5 sm:p-6 flex flex-col justify-between h-full">
    <!-- Header: Title, View Tabs & Chart Type Selector -->
    <div class="flex flex-col gap-3 mb-4 pb-3 border-b border-slate-100/80">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          ...
          <p class="text-[11px] text-slate-400 font-normal mt-0.5">
            Project hours &amp; work location distribution &bull; <span class="text-koala-blue-800 font-medium">{{ formattedDateRange }}</span>
          </p>
        </div>
      ...
      <!-- Main Filter Tabs & Sub Filter -->
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-1 bg-white/70 backdrop-blur-sm p-1 rounded-xl border border-white/90 shadow-glossy-sm">
          <button 
...
// View subcategories configuration
const viewConfig = {
  project: {
...
const currentSubcategories = computed(() => viewConfig[activeView.value].labels)
const activeSubcategories = ref([...viewConfig['project'].labels])

// Reset active subcategories when view changes
watch(activeView, (newView) => {
  activeSubcategories.value = [...viewConfig[newView].labels]
})
...
// Calculate seed factor from selected date range
...
const renderChart = () => {
  ...
  const config = viewConfig[activeView.value]
  let configType = 'bar'
  let indexAxis = 'x'
  let datasets = []
  let labels = []
  ...
  if (isAll) {
    // Grouped chart per employee (X-Axis = 5 employees)
    labels = employeeShortNames
    const groupedData = activeData.grouped[config.dataKey]
    ...
    if (chartType === 'column' || chartType === 'combo') {
      ...
      if (chartType === 'pie') {
        ...
        datasets = [{
          ...
          data: aggData,
          borderColor: '#0069A7',
          backgroundColor: 'rgba(0, 105, 167, 0.25)',
          pointBackgroundColor: '#FF8428',
    ...
    // Single selected employee mode
    const empStats = activeData.employeeStats[selectedEmployeeId.value] || activeData.employeeStats.ALL
    const rawData = (empStats[config.dataKey] || []).map(v => Math.round(v * factor * 10) / 10)
    ...
    if (chartType === 'column') {
      ...
      datasets = [{
        ...
        data: filteredData,
        borderColor: '#024981',
        backgroundColor: 'rgba(2, 73, 129, 0.2)',
        pointBackgroundColor: '#024981',
        pointBorderColor: '#ffffff',
        pointRadius: 4.5,
        tension: 0.4,
        fill: true
      ...
      datasets = [
        ...
        { type: 'line', label: 'Average', data: filteredData.map(() => Math.round(avg * 10) / 10), borderColor: '#BA0404', backgroundColor: '#BA0404', borderWidth: 2, pointRadius: 4, fill: false, order: 1 }
      ]
    }
  }
...
watch([activeView, selectedChartType, selectedEmployeeId, dateRange], () => {
  nextTick(() => renderChart())
}, { deep: true })
